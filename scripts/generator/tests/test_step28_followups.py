...
# =====================================================================
# Finding 1 — convert dispatcher skip-locked exit
# =====================================================================


def test_convert_dispatcher_origin_locked_exit_skips_finish_event(
    tmp_path, monkeypatch,
):
    """When process_convert_job returns (True, "origin_locked_by:..."),
    the dispatcher must NOT fire CONVERT_OK (would IllegalTransition
    from DEAD) and must NOT call _resume_deferred_triage."""
    ...
    # Stub state DB + helpers the dispatcher reaches for.
    db_path = tmp_path / "state.db"
    conn = sqlite3.connect(str(db_path), check_same_thread=False,
                           isolation_level=None)
    conn.row_factory = sqlite3.Row
    init_db(conn)
    # Seed a jobs row in CONVERTING so SKIP_ORIGIN_LOCKED has a
    # valid prior state to transition from.
    conn.execute(
        """INSERT INTO jobs (job_id, state, type, origin, target,
                             last_seen_at)
           VALUES ('convert-test.job', 'converting', 'convert',
                   'devel/locked', '@2026Q2', ?)""",
        (_now(),),
    )

    transitions_fired: list[JobEvent] = []

    def _track_transition(job_id, event, **kw):
        transitions_fired.append(event)
        return True

    resume_calls: list[tuple] = []
    monkeypatch.setattr(rm, "_apply_transition", _track_transition)
    monkeypatch.setattr(
        rm, "_resume_deferred_triage",
        lambda *a, **kw: resume_calls.append(a),
    )

    # Mimic the dispatcher's convert branch with the skip-locked
    # return value. The fix gates the post-call block on
    # `not status.startswith("origin_locked_by:")`.
    success, status = True, "origin_locked_by:b-staker"
    origin_locked_exit = (
        success and status.startswith("origin_locked_by:")
    )
    if not origin_locked_exit:
        rm._apply_transition(
            "convert-test.job",
            JobEvent.CONVERT_OK if success else JobEvent.CONVERT_GAVE_UP,
        )
        if success:
            rm._resume_deferred_triage(
                tmp_path, "convert-test.job", "devel/locked", "@2026Q2",
            )

    conn.close()
    assert transitions_fired == [], (
        "skip-locked convert exit must not fire any finish transition"
    )
    assert resume_calls == [], (
        "skip-locked convert exit must not trigger _resume_deferred_triage"
    )


def test_convert_dispatcher_normal_exit_still_fires_convert_ok(monkeypatch):
    """Regression guard: non-skip-locked convert success still fires
    CONVERT_OK + _resume_deferred_triage. Ensures the gate only
    affects the new short-circuit path."""
    import dportsv3.agent.runner as rm
    from dportsv3.agent.lifecycle import JobEvent

    transitions: list[JobEvent] = []
    resume_calls: list[tuple] = []

    monkeypatch.setattr(
        rm, "_apply_transition",
        lambda job_id, event, **kw: transitions.append(event) or True,
    )
    monkeypatch.setattr(
        rm, "_resume_deferred_triage",
        lambda *a, **kw: resume_calls.append(a),
    ...
    if not origin_locked_exit:
        ...
        if success:
            rm._resume_deferred_triage(None, "convert.job", "x", "y")

    assert transitions == [JobEvent.CONVERT_OK]
