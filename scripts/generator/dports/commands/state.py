...
def _state_import(config: Config, args: Namespace) -> int:
    ...
    log.info(f"Importing state from STATUS files for {target}")

    state = import_from_status_files(config, target)
    state.save()
...
def _state_export(config: Config, args: Namespace) -> int:
    ...
    target = validate_target(args.target)
    log.info(f"Exporting state to STATUS files for {target}")
    ...
    count = 0
    for port_state in state.iter_all():
        if port_state.target == target or not port_state.target:
