...
def cmd_verify(config: Config, args: Namespace) -> int:
    ...
    if args.port == "all" or args.port is None:
        log.info(f"Verifying all port patches for {quarterly}")
        ...
        for overlay in discover_overlays(ports_base):
            if overlay.manifest.has_diffs:
                diffs = overlay.get_diffs_for_quarterly(quarterly)
                
                for diff_file in diffs:
                    target_dir = config.get_freebsd_port_path(str(overlay.origin), quarterly)
        ...
        diffs = overlay.get_diffs_for_quarterly(quarterly)
        target_dir = config.get_freebsd_port_path(str(origin), quarterly)
