...
from dports.migrate import generate_overlay_toml
...
def cmd_add(config: Config, args: Namespace) -> int:
    ...
    content = generate_overlay_toml(
        ...
        description=f"DragonFly customizations for {origin}",
    )
    ...
    log.info("  - Create Makefile.DragonFly for Makefile modifications")
    log.info("  - Create diffs/ directory for patch files")
    log.info("  - Create dragonfly/ directory for overlay files")
