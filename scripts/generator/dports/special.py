...
- Diffs are target-scoped: Mk/diffs/@<target>/*.diff
"""
...
from dports.quarterly import find_invalid_target_dirs, target_dirname, validate_target
...
def apply_special_patches(
    ...
    for dirname in SPECIAL_DIRS:
        ...
        if diffs_dir.exists():
            # Root diffs are forbidden in branch-scoped design
            root_diffs = sorted(diffs_dir.glob("*.diff")) + sorted(
                diffs_dir.glob("*.patch")
            )
            if root_diffs:
                names = ", ".join(f.name for f in root_diffs)
                raise SpecialError(
                    f"Forbidden root special diffs in {diffs_dir}: {names}"
                )

            invalid_dirs = find_invalid_target_dirs(diffs_dir)
            if invalid_dirs:
                raise SpecialError(
                    f"Invalid target directories in {diffs_dir}: {', '.join(invalid_dirs)}"
                )

            target_diffs_dir = diffs_dir / target_dirname(normalized_target)
            if not target_diffs_dir.exists():
                raise SpecialError(
                    f"Missing target special diffs directory: {target_diffs_dir}"
                )

            for diff_file in sorted(target_diffs_dir.glob("*.diff")) + sorted(
                target_diffs_dir.glob("*.patch")
            ):
                ...
                if not dry_run:
                    ...
                    if full_target.exists():
                        success = apply_patch(
                            diff_file, full_target.parent, filename=full_target.name
                        )
                        if success:
...
def list_special_contents(config: Config) -> dict[str, dict[str, list[str]]]:
    ...
    for dirname in SPECIAL_DIRS:
        ...
        for item in special_dir.iterdir():
            if item.name == "diffs":
                diffs_dir = item
                diff_files = sorted(diffs_dir.rglob("*.diff")) + sorted(
                    diffs_dir.rglob("*.patch")
                )
                for diff_file in diff_files:
                    contents["diffs"].append(str(diff_file.relative_to(diffs_dir)))
            elif item.is_file():
...
def merge_infrastructure(
    ...
    fports = config.get_freebsd_ports_path(normalized_target)
    ...
    results["Mk"] = _merge_mk_templates(
        fports, merged, delta, "Mk", normalized_target, dry_run, log
    )
    results["Templates"] = _merge_mk_templates(
        fports, merged, delta, "Templates", normalized_target, dry_run, log
    )

    # Merge treetop files (UIDs, GIDs, MOVED, etc.)
    results["treetop"] = _merge_treetop(
        fports, merged, delta, normalized_target, dry_run, log
    )
...
def _merge_mk_templates(
    ...
    try:
        ...
        # Apply target-scoped patches from special/<dirname>/diffs/@<target>/
        diffs = delta / "special" / dirname / "diffs"
        if diffs.exists():
            root_diffs = sorted(diffs.glob("*.diff")) + sorted(diffs.glob("*.patch"))
            if root_diffs:
                names = ", ".join(f.name for f in root_diffs)
                raise SpecialError(f"Forbidden root special diffs in {diffs}: {names}")

            invalid_dirs = find_invalid_target_dirs(diffs)
            if invalid_dirs:
                raise SpecialError(
                    f"Invalid target directories in {diffs}: {', '.join(invalid_dirs)}"
                )

            target_dir = diffs / target_dirname(target)
            if not target_dir.exists():
                raise SpecialError(
                    f"Missing target special diffs directory: {target_dir}"
                )

            for diff_file in sorted(target_dir.glob("*.diff")) + sorted(
                target_dir.glob("*.patch")
            ):
        ...
        if replacements.exists():
            for item in replacements.rglob("*"):
                if item.is_file():
                    ...
                    replacement_target = dst / rel
                    replacement_target.parent.mkdir(parents=True, exist_ok=True)
                    shutil.copy2(item, replacement_target)
...
def _merge_treetop(
    fports: Path, merged: Path, delta: Path, target: str, dry_run: bool, log
) -> bool:
    ...
    try:
        ...
        # Apply target-scoped treetop patches
